...
class BrainNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(str("Successfully created a brain node with MTD VANILLA driving policy"))

    def execute_callback(self, lidar_scan):
        lidar_scan = lidar_scan.ranges
        #############################################
        t0 = time.time()
        full_lidar = full_lidar.tolist()


        #TODO A CHANGER CETTE MERDE HARDCODÉ HORRIBLE ALERTE ALERTE
        half_angle_deg = -75
        half_angle_rad = -75 * np.pi/180
        angular_resolution_deg = len(lidar)/360
        angular_resolution_rad = angular_resolution_deg * np.pi/360

        aperture = angular_resolution_deg * 75
        ...
        aperture_tight_front = aperture = angular_resolution_deg * 75
