...
class LidarPreprocessingNode(Node):
    ...
    def cmd_scan_callback(self, message):
        ...
        # SI ON VEUX PLOT LE LIDAR POUR VOIR COMMENT CA SE PASSE
        # self.get_logger().info(f"RAW SAMPLED LIDAR :" +  str([f"{i:.2f}" for i in lidar_scan.tolist()[::30]]))


        raw_lidar = np.array(lidar_scan.tolist())
        
        if len(raw_lidar[~np.isfinite(raw_lidar)])==len(raw_lidar):
...
#         filtered_lidar = [np.median(chunk) for chunk in filtered_lidar]
        ...
        self.get_logger().info(f"RAW SAMPLED LIDAR :" +  str(len(filtered_lidar)))
